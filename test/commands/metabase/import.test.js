...
describe('metabase:import', () => {
    let queImpdata, resultQueList = [], resultDashList = [], queExistance = false, dashExistance = false, dashImpdata,
    ...
    before(async () => {
        if (!fs.existsSync(queImpFilePath) || path.extname(queImpFilePath) !== '.json') {
            throw new CredError('Provided a valid path to the json file.', { suggestions: [`Run command ${chalk.yellowBright(`credcli metabase:import`)}`] });
        }
    ...
    describe("Importing dashboard", () => {
        ...
        it("Tagging and publish status check of imported questions", () => {
            let questionInImpDash = dashImpdata.data["ordered_cards"].map(x => x), recentImportedQuestions = [];
